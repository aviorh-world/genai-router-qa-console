# GenAI Router QA Console 0.02.5 — Vercel Diagnostic
...
## 0.02.5 diagnostic hotfix
- Exact Postman UI no longer assumes every Vercel response is JSON. HTML/403/404 responses are shown as diagnostics instead of `Unexpected token <`.
- Exact Postman API decodes non-secret JWT claims locally for diagnostics (aud/iss/email/exp only; token remains redacted).
- Adds targeted hints for expired token, audience mismatch, Cloud Run 403/Invoker permission, and HTML upstream errors.
